...
Змяніце кампанент `Avatar` так, каб запытваць відарыс найбольш падыходзячага памеру, базуючыся на пропсе `size`. Канкрэтна, калі `size` меней за `90`, перадайце `'s'` («small») замест `'b'` («big») у функцыю `getImageUrl`. Пераканайцеся, што вашыя змены працуюць, паспрабаваўшы адрэндэрыць аватары з рознымі значэннямі пропса `size` і адкрыць выяву ў новай укладцы.
