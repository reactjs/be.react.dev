...
Зборнік Next.js App Router цалкам рэалізуе афіцыйную [спецыфікацыю серверных кампанентаў React](https://github.com/reactjs/rfcs/blob/main/text/0188-server-components.md). Гэта дазваляе змяшаць кампаненты часу зборкі, серверныя і інтэрактыўныя кампаненты ў адным дрэве React.
