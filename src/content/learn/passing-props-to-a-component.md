...
Пропсы, якія можна перадаць у тэг `<img>` прадвызначаныя (ReactDOM адпавядае [стандарту HTML](https://www.w3.org/TR/html52/semantics-embedded-content.html#the-img-element)). Але ва *ўласныя кампаненты*, такія як `<Avatar>`, вы можаце перадаваць любыя пропсы, каб іх кастамізаваць. Вось як гэта рабіць!

## Перадача пропсаў у кампаненты {/*passing-props-to-a-component*/}

У гэтым кодзе кампанент `Profile` не перадае аніводнага пропса свайму дачыннаму кампаненту `Avatar`:
...
### Крок 1: Перадаць пропсы дачыннаму кампаненту {/*step-1-pass-props-to-the-child-component*/}
...
Калі падвойныя фігурныя дужкі пасля `person=` вас блытаюць, напамінаем, што [гэта ўсяго толькі аб’екты](/learn/javascript-in-jsx-with-curly-braces#using-double-curlies-css-and-other-objects-in-jsx) ўнутры дужак JSX.

</Note>

Цяпер вы можаце прачытаць гэтыя пропсы знутры кампанента `Avatar`.

### Крок 2: прачытайце пропсы знутры дачыннага кампанента {/*step-2-read-props-inside-the-child-component*/}

Вы можаце прачытаць гэтыя пропсы, пералічыўшы іх назвы `person, size` цераз коску ўнутры `({` і `})` адразу пасля `function Avatar`. Гэта дазволіць вам выкарыстоўваць іх знутры кода `Avatar`, нібы яны пераменныя.
...
Пропсы дазваляюць вам успрымаць бацькоўскі і дачынны кампаненты незалежнымі адзін ад аднаго. Напрыклад, вы можаце змяніць пропсы `person` ці `size` унутры кампанента `Profile` нават не задумваючыся ад тым, як кампанент `Avatar` іх выкарыстоўвае. Аналагічна, вы можаце змяняць тое, як `Avatar` апрацоўвае гэтыя пропсы, не гледзячы на логіку кампанента `Profile`.
...
Звычайна вам не будзе патрэбны сам аб’ект `props`, прасцей будзе разабраць яго на пропсы па адным адразу.
...
## Перадача пропсаў з карыстаннем сінтаксіса разгортвання {/*forwarding-props-with-the-jsx-spread-syntax*/}
...
**Карыстайцеся аператарам разгортвання разумна.** Калі вы пачынаеце выкарыстоўваць яго ў кожным кампаненце, вы штосьці робіце не так. Часта гэта прыкмета таго, што варта раздзяліць кампаненты і перадаць дачынныя ў выглядзе JSX. Далей разгледзім як гэта зрабіць!

## Перадача JSX у якасці дачынных элементаў {/*passing-jsx-as-children*/}
...
**Не спрабуйце «змяняць пропсы». **Калі вы хочаце адрэагаваць на ўведзеныя карыстальнікам даныя (напрыклад, змену колера), вам спатрэбіцца «задаць стан». Падрабязней пра гэта вы можаце даведацца на старонцы «[Стан: Памяць кампанента.](/learn/state-a-components-memory)»

<Recap>

* Каб перадаць пропсы, дадайце іх у JSX як бы вы дадалі атрыбуты ў HTML.
* Каб прачытаць пропсы, скарыстайцеся дэстурызацыйным сінтаксісам: `function Avatar({ person, size })`.
...
* Вы можаце перадаць усе пропсы дачыннаму элементу, скарыстаўшыся сінтаксісам разгортвання: `<Avatar {...props} />`. Але не выкарыстоўвайце яго зашмат!
* Укладзены JSX, такі як `<Card><Avatar /></Card>`, з’явіцца ў кампаненце `Card` у якасці пропсы `children`.
* Пропсы нязменны і адлюстроўваюць толькі цяперашні стан: пры кожны рэндары кампанент атрымлівае новую версію пропсаў.
...
Іншым рашэннем, больш падобныя на папярэднія прыклады, будзе згрупаваць усе даныя пра асобу ў аб’ект і перадаць яго ў якасці аднаго пропса:
...
#### Рэгуляванне памера відарыса пропсам {/*adjust-the-image-size-based-on-a-prop*/}

У дадзеным выпадку `Avatar` атрымлівае памер у лічбах праз пропс `size`, што задае шырыню і вышыню для элемента `<img>`. У дадзеным прыкладзе пропс `size` зададзены `40`. Але калі вы адкрыеце выяву ў новай вокладцы, вы пабачыце, што памер самога відарыса значна большы (`160` пікселей). Сапраўдны памер відарыса вызначаецца памерам мініяцюры, якую вы запрашаеце.

Змяніце кампанент `Avatar` так, каб запрашваць відарыс найбольш падыходзячага памеру, базуючыся на пропсе `size`. Канкрэтна, калі `size` меней за `90`, перадайце `'s'` («small») замест `'b'` («big») у функцыю `getImageUrl`. Пераканайцеся, што вашыя змены працуюць, паспрабаваўшы адрэндэрыць аватары з рознымі значэннямі пропса `size` і адкрыць выяву ў новай укладцы.
...
Вынесіце кампанент `Card` з разметкі ніжэй і скарыстайцеся пропсам `children`, каб перадаваць у яго розную JSX разметку:
  ...
  return (
    <div>
      ...
      <div className="card">
        <div className="card-content">
          <h1>Пра навукоўца</h1>
          <p>Аклілу Лема быў выбітным Эфіопскім навукоўцам, хто адкрыў натуральны спосаб лячэння шыстасамоз.</p>
        </div>
      </div>
    </div>
  );
